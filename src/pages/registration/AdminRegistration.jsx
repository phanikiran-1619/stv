...
const AdminRegistration = () => {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/admins`, {
        headers: {
      ...
      setErrorMessage('Failed to load admins list');
    }
  };
  ...
  const fetchAdminData = async (adminId) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/admins/${adminId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleSearch = (value) => {
    ...
    const filtered = admins.filter(admin =>
      admin.stvAdminId?.toLowerCase().includes(value.toLowerCase()) ||
      `${admin.firstName} ${admin.lastName}`.toLowerCase().includes(value.toLowerCase())
    );
  ...
  const handleAdminSelect = (admin) => {
    setSelectedAdminId(admin.stvAdminId);
    setSearchTerm(`${admin.stvAdminId} - ${admin.firstName} ${admin.lastName}`);
    setShowDropdown(false);
    fetchAdminData(admin.stvAdminId);
  };
  ...
  const validateField = useCallback((name, value) => {
    switch (name) {
      ...
      case 'password':
        ...
        if (value && value.length !== 10) return 'Must be exactly 10 characters';
        if (value && !/[A-Z]/.test(value)) return 'Must contain at least one uppercase letter';
        return '';
      case 'contactNum':
  ...
  const validateForm = useCallback(() => {
    ...
    if (isEditMode && !selectedAdminId) {
      setErrorMessage('Please select an admin to update');
      return false;
    }
  ...
  }, [formData, isEditMode, selectedAdminId, validateField]);
  ...
  const handleChange = useCallback((e) => {
    ...
    if (type === 'checkbox') {
      ...
      // Allow alphanumeric for username
      if (value.length <= 30) {
        setFormData(prev => ({ ...prev, [name]: value }));
      }
    } else if (name === 'password') {
      // Allow alphanumeric for password
      if (value.length <= 10) {
        setFormData(prev => ({ ...prev, [name]: value }));
      }
    } else {
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      const url = isEditMode
        ? `${process.env.REACT_APP_API_BASE_URL1}/api/v1/admins/${selectedAdminId}`
        : `${process.env.REACT_APP_API_BASE_URL1}/api/v1/admins`;
      ...
      if (!response.ok) {
        throw new Error(responseData.message || (isEditMode ? 'Failed to update admin' : 'Failed to register admin'));
      }
      ...
      if (isEditMode) {
        ...
        setSuccessMessage(`Registration successful! Admin ID: ${responseData.stvAdminId}`);
      }
  ...
  // Initialize admins on edit mode
  useEffect(() => {
    if (isEditMode) {
      fetchAdmins();
    }
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-8">
            <h1 className="text-4xl font-bold bg-gradient-to-r from-purple-600 via-purple-700 to-purple-800 bg-clip-text text-transparent mb-2">
              {isEditMode ? 'Update Admin' : 'Admin Registration'}
            </h1>
            <p className="text-gray-600 dark:text-gray-300">
              {isEditMode ? 'Update existing admin profile' : 'Register a new admin profile'}
            </p>
          </div>
          ...
          <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl rounded-2xl p-8 border-2 border-purple-200/70 dark:border-purple-700/70 shadow-2xl">
                ...
                <div className="relative" ref={dropdownRef}>
                  <div className="relative">
                    <input
                      ...
                      placeholder="Search admin by ID or name..."
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    disabled={isEditMode && !selectedAdminId}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        placeholder="Enter password (10 chars, 1 uppercase)"
                        maxLength={10}
                        required
                      />
                      <button
              ...
              <div className="space-y-4">
                ...
                <Button
                  type="submit"
                  disabled={isLoading || (isEditMode && !selectedAdminId)}
